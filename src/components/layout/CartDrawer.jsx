        ...
        <AnimatePresence mode="wait">
          {couponCode ? (
            <motion.div key="saved"
              ...
              className="flex items-center justify-between bg-emerald-50 border border-emerald-100 rounded-2xl px-3.5 py-2.5">
            ...
            <motion.div key="input" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div className="flex gap-2">
                <input
                  ...
                  maxLength={20}
                  disabled={loading}
                  className={`flex-1 font-mono text-sm font-semibold tracking-wider bg-white border rounded-full px-4 py-2.5 uppercase placeholder-ink-300 placeholder:font-sans placeholder:font-normal placeholder:tracking-normal focus:outline-none focus:ring-2 transition-all disabled:opacity-50 ${
                ...
                <motion.button whileTap={{ scale: 0.95 }} onClick={apply}
                  ...
                  className="px-5 py-2.5 bg-ink-900 hover:bg-rose-500 text-white text-xs font-bold rounded-full transition-colors disabled:opacity-40 whitespace-nowrap">
                  {loading
      ...
      {isOpen && (
        <>
          ...
          <motion.div key="bd"
            ...
            style={{ background: 'rgba(35,26,27,0.45)', backdropFilter: 'blur(4px)' }}
            onClick={closeCart} />
          ...
          <motion.div key="drawer"
            ...
            className="fixed right-0 top-0 h-full w-full max-w-[420px] z-[70] flex flex-col shadow-modal"
            style={{ background: '#FFFBF7' }}>

            {/* ── Header ── */}
            <div className="px-5 pt-5 pb-4 flex-shrink-0">
              <div className="flex items-center justify-between mb-1">
                <h2 className="font-display font-semibold text-ink-900 text-2xl leading-none">Tu pedido</h2>
                <button onClick={closeCart} aria-label="Cerrar carrito"
                  className="w-9 h-9 rounded-full bg-cream-100 hover:bg-cream-200 text-ink-500 hover:text-ink-900 flex items-center justify-center transition-colors">
                  <CloseIcon />
                </button>
              </div>
              <p className="text-[13px] text-ink-400">
                {items.length === 0
                  ...
                  : `${items.length} ${items.length === 1 ? 'producto' : 'productos'} · ${formatCRC(total)}`}
              </p>

              {/* Flujo compacto — solo visible cuando hay items */}
              {items.length > 0 && (
                <div className="mt-4 flex items-center gap-0 bg-white rounded-2xl px-4 py-3 border border-cream-200 shadow-soft">
                  {[
                    ...
                    <div key={s.n} className="flex items-center flex-1 min-w-0">
                      <div className="flex flex-col items-center gap-1 flex-1">
                        <span className="w-6 h-6 rounded-full text-white text-[11px] font-bold flex items-center justify-center flex-shrink-0"
                          style={{ background: 'linear-gradient(135deg, #CE6C8D 0%, #B25577 100%)' }}>
                          {s.n}
                        </span>
                        <span className="text-[10px] font-semibold text-ink-500 text-center leading-tight">{s.label}</span>
                      </div>
                      {i < 2 && (
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-cream-300 flex-shrink-0 mb-3">
                          <polyline points="9 18 15 12 9 6"/>
                        </svg>
                      )}
                    </div>
            ...
            <div className="h-px bg-cream-200 mx-5" />
              ...
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-5 text-center py-12">
                  <motion.div
                    ...
                    className="w-20 h-20 rounded-3xl bg-rose-50 border border-rose-100 flex items-center justify-center text-rose-400 shadow-soft">
                    <BagIcon />
                  </motion.div>
                  <div>
                    <p className="font-display font-semibold text-ink-800 text-lg mb-1">Tu carrito está vacío</p>
                    <p className="text-ink-400 text-sm leading-relaxed max-w-[200px]">
                      Elegí productos y te coordinamos el envío por WhatsApp
                    </p>
                  </div>
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <motion.div key={item.id || item._id}
                        ...
                        className="flex gap-3.5 p-3.5 rounded-2xl bg-white border border-cream-200 shadow-soft">

                        {/* Image */}
                        <div className="w-[68px] h-[68px] rounded-xl overflow-hidden bg-cream-100 flex-shrink-0 border border-cream-200">
                          {img
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                          <div>
                            <p className="text-[14px] font-semibold text-ink-900 leading-snug line-clamp-2 mb-0.5">{item.name}</p>
                            <p className="text-[11px] text-rose-500 font-medium tracking-wide uppercase">{item.brand || 'Be Curlyful CR'}</p>
                          ...
                          <div className="flex items-center justify-between mt-2">
                            {/* Qty */}
                            <div className="flex items-center bg-cream-50 border border-cream-200 rounded-full overflow-hidden">
                            ...
                            <p className="font-display font-semibold text-ink-900">{formatCRC(item.price * item.qty)}</p>
                          </div>
                        </div>
            ...
            {/* ── Footer ── */}
            {items.length > 0 && (
              <div className="px-5 pt-4 pb-5 border-t border-cream-200 space-y-4 flex-shrink-0"
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-ink-900">Total estimado</span>
                    <span className="font-display font-semibold text-ink-900 text-xl tabular-nums">{formatCRC(finalTotal)}</span>
                  </div>
                ...
                <Link to="/checkout" onClick={closeCart}
                  className="flex items-center justify-center gap-2.5 w-full bg-[#25D366] hover:bg-[#1fbc59] text-white font-bold py-4 rounded-full transition-colors shadow-card text-[15px]">
                  <WaIcon />
                  Pedir por WhatsApp
                </Link>

                <p className="text-center text-[11px] text-ink-400 leading-relaxed">
                  Completás un formulario rápido → confirmamos por WhatsApp → pagás por SINPE
                </p>
              </div>
            )}
          </motion.div>
